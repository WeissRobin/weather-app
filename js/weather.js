...
showAutocomplete = (data) => {
    ...
    cities.forEach((city) => {
        city.addEventListener('click', (e) => {
            ...
            fetch(`http://api.weatherapi.com/v1/forecast.json?key=b301fae74e5c4e42926164325232512&q=${cityInput.value}&days=7&aqi=no&alerts=no`)
            .then(data => data.json())
...
showUI = (city) => {
    ...
    ${city.forecast.forecastday.map(forecast => {
        return `
        <div class="forecastday">
            <span class="forecast-day">${days[new Date(forecast.date).getDay()]}</span>
            <img class="forecast-image" src="${forecast.day.condition.icon}">
            <span class="forecast-condition">${forecast.day.condition.text} <p class="forecast-celsius"><span class="max-c">${forecast.day.maxtemp_c}</span><span class="min-c">/${forecast.day.mintemp_c}</span></p>
        </div>
        `;
    }).join('')}
    `;
    welcomeUI.style.animation = 'SHRINK_MAIN 2s forwards';
    weatherUI.style.animation = 'EXPAND_WEATHER 2s forwards';
}
